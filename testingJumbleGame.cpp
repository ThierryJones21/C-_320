...
void testJumble() {
	...
	// My own test cases, testing various cases

	//Test difficulty settings, along with getters
	cout << "\nTesting getROW" << endl;
	cout << "\nTesting getCOL" << endl;
	cout << "\nTesting getDIRECTION" << endl;
	cout << "\nTesting getSIZE" << endl;
	cout << "\nSizes should be (16, 24, 32)" << endl;

	//easy difficulty
	JumblePuzzle easy("TESTCASE", "easy");
	showJumble(easy.getJumble(), easy.getSize());
	cout << "Row: " << easy.getRowPos() << " Col: " << easy.getColPos() << " Direction: " << easy.getDirection() << " Size: " << easy.getSize() << endl;
	cout << "Easy puzzle" << endl;

	//medium difficulty
	JumblePuzzle medium("TESTCASE", "medium");
	showJumble(medium.getJumble(), medium.getSize());
	cout << "Row: " << medium.getRowPos() << " Col: " << medium.getColPos() << " Direction: " << medium.getDirection() << " Size: " << medium.getSize() << endl;
	cout << "Medium puzzle" << endl;
	//hard difficulty
	JumblePuzzle hard("TESTCASE", "hard");
	showJumble(hard.getJumble(), hard.getSize());
	cout << "Row: " << hard.getRowPos() << " Col: " << hard.getColPos() << " Direction: " << hard.getDirection() << " Size: " << hard.getSize() << endl;
	cout << "Hard puzzle" << endl;
	try {
		cout << "\nWord that is too big, should throw error" << endl;
		JumblePuzzle jp5("ABigWordVeryBigWord", "easy");
	}
	catch (BadJumbleException e)
	{
		cout << e.what() << endl;
	}
	//Word that is too small
	try {
		cout << "\nWord that is too small, should throw error" << endl;
		JumblePuzzle jp6("No", "easy");
	}
	catch (BadJumbleException e)
	{
		cout << e.what() << endl;
	}
	//Difficulty that is not listed
	try {
		cout << "\nDifficuty that is not listed" << endl;
		JumblePuzzle jp7("WORD", "medium-hard");
	}
	catch (BadJumbleException e)
	{
		cout << e.what() << endl;
